...
enum SummaryPromptBuilder {
    ...
    private static func isLowSignal(_ record: ActivityRecord) -> Bool {
        ...
        case .fileSystem, .finder:
            let blocked = ["/.dayflow/", "/library/caches/", "/library/logs/",
    ...
    private static func dedupeWithinWindow(_ activities: [ActivityRecord], windowSeconds: TimeInterval) -> [ActivityRecord] {
        var lastSeen: [String: Date] = [:]
        var result: [ActivityRecord] = []
        for record in activities {
            guard record.type == .fileSystem || record.type == .finder || record.type == .window else {
                result.append(record)
                continue
            }
